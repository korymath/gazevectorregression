function [eyeData,markerData,transformMatrix,offset,trueMarker] = collectData(strCell)
...
for strNum = 1:length(strCell)
    ...
    headersDLAB = importDlabA83header([pwd '/raw/Dlab_' strCell{strNum} '.csv']);
    ...
    dataDLAB = importDlabA83([pwd '/raw/Dlab_' strCell{strNum} '.csv'],format);
...
for strNum = 1:length(strCell)
    ...
    C = importViconA83header([pwd '/raw/Vicon_' strCell{strNum} '.csv']);
    ...
    iV = [find(strcmp(headersVICON,'Wand:Tip')),...
        find(~cellfun(@isempty,regexp(headersVICON,'RFHD'))),...
        find(~cellfun(@isempty,regexp(headersVICON,'RBHD'))),...
        find(~cellfun(@isempty,regexp(headersVICON,'LFHD'))),...
        find(~cellfun(@isempty,regexp(headersVICON,'LBHD')))];
    ...
    dataVICON = importViconA83([pwd '/raw/Vicon_' strCell{strNum} '.csv'],format);
